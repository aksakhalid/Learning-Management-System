...
<form action=" Teacher Time table.php" method="post" >
  <p align="center"><u class="style1">Teacher  Time table</u></p>
  <p align="center">&nbsp;</p>
...
<center>
     ...
     <table width="1200px" style = 'margin-left:100px; margin-top: 30px; border-color: #000033; text-align:center;' border = "10">
         <tr>
           <td bgcolor="#FFFFFF" style="background: #990033; color:#FFFFFF"><strong>Teacher ID </strong></td>
           <td style="background: #990033; color:#FFFFFF"><strong>Teacher Name</strong></td>
...
if (mysqli_connect_errno())
  ...
  echo "Failed to connect to MySQL: " . mysqli_connect_error();
	...
	$sql = "SELECT tid,  tname,monday , tuesday , wednesday , thursday , friday , saturday FROM teachertimetable ";
   	$result = $con->query($sql);
	if ($result->num_rows > 0) {
  	
    // output data of each row
  				  while($row = $result->fetch_assoc()) {
					echo "<tr>  <td>".$row['tid']."</td>  <td>".$row['tname']."</td> <td>".$row['monday']."</td> <td>".$row['tuesday']."</td> <td>".$row['wednesday']."</td>  <td>".$row['thursday']."</td> <td>".$row['friday']."</td> <td>".$row['saturday']."</td>  </tr>";
    }
               echo "</table>";
}
